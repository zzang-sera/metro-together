...
export default function Line7RouteScreen() {
  const route = useRoute();
  const { stationName = "건대입구", type = "EV", imageUrl = null } = route.params || {};
  ...
  return (
    <View style={s.container}>
      <Text style={s.header}>
        {stationName} - {getTypeLabel(type)} 좌표 등록 (7호선)
      </Text>
      ...
      >
        ...
        <Svg style={[s.overlay, { width: imgLayout.width, height: imgLayout.height }]}>
          {tempPoints.map((p, i) => (
            <Circle key={i} cx={p.cx} cy={p.cy} r={10} fill={TYPE_COLORS[p.type]} stroke="#fff" strokeWidth={2} />
          ))}
        </Svg>
      </Animated.View>
    </View>
...
function getTypeLabel(t) {
  const names = {
    EV: "엘리베이터", ES: "에스컬레이터", TO: "화장실",
    DT: "장애인 화장실", WL: "휠체어 리프트", VO: "음성유도기",
    NU: "수유실", LO: "보관함"
  };
  return names[t] || "시설";
}
...
const s = {
  ...
  btn: {
    backgroundColor: "#14CAC9", paddingVertical: 8, paddingHorizontal: 12,
    borderRadius: 10, alignSelf: "center", margin: 6,
  },
