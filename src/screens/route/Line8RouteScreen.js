...
export default function Line8RouteScreen() {
  const route = useRoute();
  const { stationName = "잠실", type = "EV", imageUrl = null } = route.params || {};
  ...
  return (
    <View style={s.container}>
      <Text style={s.header}>
        {stationName} - {getTypeLabel(type)} 좌표 등록 (8호선)
      </Text>
...
function getTypeLabel(t) {
  const names = {
    ...
    DT: "장애인 화장실", WL: "휠체어 리프트",
    VO: "음성유도기", NU: "수유실", LO: "보관함"
  };
  return names[t] || "시설";
}
