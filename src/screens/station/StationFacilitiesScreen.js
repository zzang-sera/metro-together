...
export default function StationFacilitiesScreen() {
  ...
  useEffect(() => {
    console.log("[NAV] StationFacilities mounted", { stationCode, stationName, line, type });
  }, [stationCode, stationName, line, type]);
  ...
  useEffect(() => {
    if (!type) return; // 그리드 모드일 때는 로드하지 않음
    let cancelled = false;
    async function load() {
      setItems(null);
      if (type === TYPES.ELEVATOR) {
        ...
        const count = type === TYPES.ACCESSIBLE_TOILET ? 2 : 5;
        if (!cancelled) setItems(mockItems(count, typeLabel));
      }
    }
    load();
    return () => { cancelled = true; };
  }, [type, stationCode, line, typeLabel]);
      ...
      {!type ? (
        ...
        <View style={styles.listWrap}>
          <View style={styles.listHeader}>
            <Text style={[styles.listTitle, { fontSize: responsiveFontSize(16) + fontOffset }]}>
              {typeLabel} 목록
            </Text>
          ...
          {items === null ? (
            <View style={styles.loadingWrap}>
              <ActivityIndicator />
              <Text style={[styles.loadingText, { fontSize: responsiveFontSize(12) + fontOffset }]}>불러오는 중…</Text>
            </View>
          ) : items.length === 0 ? (
            <View style={styles.emptyWrap}>
              <Text style={[styles.emptyText, { fontSize: responsiveFontSize(14) + fontOffset }]}>표시할 항목이 없어요</Text>
            </View>
          ) : (
            <FlatList
              data={items}
              ...
              renderItem={({ item }) => (
                <View style={styles.card}>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.cardTitle, { fontSize: responsiveFontSize(16) + fontOffset }]}>{item.title}</Text>
                    {!!item.desc && <Text style={[styles.cardDesc, { fontSize: responsiveFontSize(12) + fontOffset }]}>{item.desc}</Text>}
                    {!!item.line && <Text style={[styles.cardMeta, { fontSize: responsiveFontSize(12) + fontOffset }]}>라인: {item.line}</Text>}
                  </View>
                  <View style={[
                    styles.badge2,
                    item.status === "사용가능" || item.status === "정상" ? styles.ok :
                    /중/.test(item.status) ? styles.warn : styles.neutral
                  ]}>
                    <Text style={[styles.badgeText2, { fontSize: responsiveFontSize(12) + fontOffset }]}>{item.status}</Text>
                  </View>
                </View>
              )}
            />
...
const styles = StyleSheet.create({
  ...
  card: {
    ...
    borderWidth: 1, borderColor: "#EEF2F7", marginHorizontal: 12,
  },
